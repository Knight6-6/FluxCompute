...
现阶段 `Executor` 按拓扑序**顺序**执行：Input 节点由外部注入数据，Operator 节点收集上游输出并调用绑定好的闭包，Output 节点零成本透传（与上游是同一个张量对象）。`ThreadPool` 已就位但尚未接入，按依赖层并行调度是下一步。

节点输出以共享句柄按 node id 存放，沿边传递只拷贝指针。`get_output` 返回持有所有权的句柄，只要调用方还持有它，张量就一直有效——跨多次 `run()` 也成立。
