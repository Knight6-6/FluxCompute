...
void shift_axis(const T* input, T* output, 
                ...
                std::ptrdiff_t offset, T fill_value) {
    
    for (std::size_t o = 0; o < outer_size; ++o) {
        for (std::size_t a = 0; a < axis_size; ++a) {

            std::ptrdiff_t src_a = static_cast<std::ptrdiff_t>(a) - offset;

            T* out_ptr = output + (o * axis_size + a) * inner_size;

            if (src_a >= 0 && src_a < static_cast<std::ptrdiff_t>(axis_size)) {

                const T* in_ptr = input + (o * axis_size + static_cast<std::size_t>(src_a)) * inner_size;
