...
public:
    ...
    // -----------------------------------------------------------------------
    // 异步执行：提交即返回，计算在 pool 的后台线程上进行。
    //
    //     auto fut = ex.run_async(graph, pool);
    //     ... 干别的 ...
    //     fut.get();                      // 等待；节点里抛的异常在这里重抛
    //     auto out = ex.get_output(node); // 就绪后才能取结果
    //
    // 与 run / run_parallel 的关系——三者按**两个正交维度**划分，不是互斥模式：
    //
    //                 同步（调用方等）      异步（提交即返回）
    //     顺序执行      run()                run_async()
    //     图内并行      run_parallel()        ——（有意不提供）
    //
    // 之所以不做成 run(mode) 那样的模式参数：这两个维度是独立的，"等不等结果"
    // 和"图内要不要并发"互不决定。塞进一个开关既表达不了组合，又会让
    // run(SYNC) 与 run_parallel 变成两个入口做同一件事。
    //
    // -----------------------------------------------------------------------
    // 为什么没有"异步 + 图内并行"
    //
    // 实测：向同一个固定大小的线程池**嵌套提交**会死锁。2 线程的池、2 个外层
    // 任务各提交 1 个子任务，最终完成 0/4——两个 worker 都卡在等自己的子任务，
    // 而没有线程能去跑那些子任务。run_async 若内部再调 run_parallel(pool)，
    // 就是这个形状。
    //
    // 而且即便不死锁，两处并行叠加也会让线程数超额订阅，通常得不偿失。
    //
    // 需要"同时跑多组任务"时，正确的用法是每组内部顺序执行、让多个任务在池里
    // 自然并发：
    //
    //     for (auto& 参数组 : 参数网格)
    //         futures.push_back(ex->run_async(graph, pool));   // 池有 N 个线程
    //                                                            // 就有 N 组同时在跑
    //
    // -----------------------------------------------------------------------
    // 调用方的两条契约（无法在库内自动检查，见下）
    //
    // 1. future 就绪前不要碰这个 Executor：set_input / get_output / 再次 run
    //    都会与后台线程竞争 values_，是数据竞争。等价于"没 run 就取结果"，
    //    只是异步下不会立刻抛异常，而是静默读到半成品。
    // 2. *this 与 graph 必须活到 future 就绪。
    //
    // 为什么不做运行时检查（比如一个 busy 标志）：那需要给 Executor 加一个
    // 原子成员，而原子不可拷贝——会破坏 Executor 可拷贝这一条已经被
    // static_assert 守住的属性（见 test_executor.cpp）。为一个调用方的逻辑
    // 错误付这个代价不划算，故用契约 + 测试锁定。
    // -----------------------------------------------------------------------
    std::future<void> run_async(const graph::Graph<T>& graph, ThreadPool& pool) {
        return pool.enqueue([this, &graph] { run(graph); });
    }
